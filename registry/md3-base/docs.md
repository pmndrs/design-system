...
- Need a colour M3 has no role for? Spread `pmndrsMtb` into your own config and add `customColors` there, rather than editing the installed file. `blend: true` harmonizes them against the pmndrs seed. Then name it in the Tailwind plugin, beside the `@import` this item installs — `@plugin "material-theme-builder/tailwind" { custom-colors: note; }` — which is what brings `bg-note`, `text-on-note`, `bg-note-container` and the eleven `bg-note-300` shades into existence. The stylesheet alone covers standard M3 roles only, and an unnamed custom colour means Tailwind emits no rule for it at all, with no error. Keep the `@import`: dropping it and letting the plugin carry everything hands `background`, `primary` and `secondary` back to shadcn, since a plugin's theme values are only defaults.
